...
class MessageBrokerService {
  ...
  async getChannel() {
    ...
    if (!this.connection) {
      ...
      this.connection.on("error", (error) => {
        logger.error("broker.connection.error", { message: error.message });
      });
  ...
  async publishEvent(queue, payload) {
    ...
    logger.info("broker.publish", { queue });
  }
  ...
  async consumeEvent(queue, callback) {
    ...
    await channel.consume(queue, async (message) => {
      ...
      try {
        ...
        logger.error("broker.consume.error", {
          queue,
    ...
    logger.info("broker.consume.start", { queue });
  }
}
